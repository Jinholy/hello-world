...
숫자가 영수의 세 자리 수에 있긴 하나 다른 자리에 위치하면 볼 한 번으로 센다. 예) 영수가 324를 갖고 있으면 429는 1 스트라이크 1 볼이다. 241은 0 스트라이크 2 볼이다. 924는 2 스트라이크 0 볼이다.
...
영수가 답한 스트라이크 개수를 나타내는 정수와 볼의 개수를 나타내는 정수, 
...
//		String input = s.nextLine();
		String input = "4 123 1 1 356 1 0 327 2 0 489 0 1";
		String tmp[] = input.split("\\s");
		
		int size = Integer.parseInt(tmp[0]);
		int tries[] = new int[size];			//1 4 7 10
		int ballCount[] = new int[size];		//2 5 8 11
		int strikeCount[] = new int[size];		//3 6 9 12
		
		for(int i=0; i<size; i++) {
			tries[i] = Integer.parseInt(tmp[1+i*3]);
			ballCount[i] = Integer.parseInt(tmp[2+i*3]);
			strikeCount[i] = Integer.parseInt(tmp[3+i*3]); 
		}
		
		
		for(int i=0; i<size; i++) {
			System.out.println("try" + i +":" + tries[i] + " b/s: " + ballCount[i] + "/" + strikeCount[i]);
		}
		
		Try trs[] = new Try[size];
		ArrayList<Integer> result = new ArrayList<Integer>();
		
		for(int i=0; i<size; i++) {
			trs[i]= new Try(tries[i], ballCount[i], strikeCount[i]);
		}
		for(int i=0; i<size; i++) {
			result.addAll(trs[i].getCases());
			System.out.print(i + ":");
			trs[i].printCases();
		}
		
		for(int i=0; i<size; i++)
			result.retainAll(trs[i].getCases());
		
		
		System.out.println("정답의 경우의수 : " +result.size());
	}
